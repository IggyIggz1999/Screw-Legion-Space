...
# Settings & Config
# ------------------------------------------------------------------------
SCRIPT_ENABLED: bool = True                                                                             # True/False decided whether this script is active
...
REPLACEMENT_EXECUTABLE_PATH: str = r"C:\\Program Files\\Playnite\\Playnite.FullscreenApp.exe"           # The full filepath to the executable that should be launched instead of Legion Space
...
if __name__ == "__main__":
    ...
    # Check all user input in the settings
    if not isinstance(SCRIPT_ENABLED, bool):
        logging.error(f"There was an error with the 'SCRIPT_ENABLED' setting: Needs to be True or False! Due to this error the script has closed.")
        sys.exit()
    if not isinstance(CHECK_FOR_PROCESS_FREQUENCY, (float, int)):
        logging.error(f"There was an error with the 'CHECK_FOR_PROCESS_FREQUENCY' setting: Needs to be a valid number of seconds! Due to this error the script has closed.")
        sys.exit()
        
    if len(LOGS_PATH) == 0:
        logging.error(f"There was an error with the 'LOGS_PATH' setting: Needs to be a valid filepath and cannot be empty! Due to this error the script has closed.")
        sys.exit()
    if len(LEGION_SPACE_EXECUTABLE_NAME) == 0:
        logging.error(f"There was an error with the 'LEGION_SPACE_EXECUTABLE_NAME' setting: Needs to be a valid executable name and cannot be empty! Due to this error the script has closed.")
        sys.exit()
    if len(REPLACEMENT_EXECUTABLE_PATH) == 0:
